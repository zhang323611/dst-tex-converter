...
- **嵌套压缩包**：压缩包内若还有压缩包，会逐层递归处理其中的纹理（可在设置中关闭）
- **原地替换 + 备份**：转换后直接替换原文件，原文件自动备份为 `.bak`
- **格式互转设置**：可选转换方向（自动 / PNG→TEX / TEX→PNG / TEX→ASTC）、ASTC 块大小、压缩质量
- **跳过提示**：已是目标格式（如已是 ASTC）或模式不符的文件会跳过，并在结果中给出计数，不会静默处理

### 预览与动画
- **`.tex` 图片预览**：直接查看纹理内容（自动处理垂直翻转存储）
- **`.dyn` 解密**：解密动画数据文件
- **动画预览**：解析 `anim.bin` / `build.bin`，播放动画、拖动帧条、切换动画、切换朝向与层序
- **动画编辑器（横屏）**：左侧元素列表、多层展开工具栏，支持拖动元素、换帧、缩放、旋转、层级、撤销，并可原地写回（原包备份为 `.bak`）

> ⚠️ **动画编辑器处于未完成状态**：目前编辑能力覆盖位置、换帧、缩放、旋转、层级与撤销；
> 增删帧、替换图集、多动画编辑等尚未实现。保存为原地字节改写，仅修改元素变换字段。

### 诊断
- **运行日志**：菜单「查看日志」可滚动查看、复制、分享或清空；同时落盘 `cacheDir/dsh_tex.log`。
  转换链路全程记录，异常附带完整调用栈，便于定位失败原因。
- **调试模式**：设置中开启后，预览画布会叠加诊断信息（渲染统计、UV 采样、图集缩略图、三角形描边）
...
- **点按 `.tex`** → 直接预览图片
- **点按 `.zip`** → 提供「预览动画 / 编辑动画」两个入口
  - 预览动画：播放、拖动帧条、切换动画/朝向/层序、翻转 UV
  - 编辑动画：进入横屏编辑器（见上方「动画编辑器」的状态说明）
- **点按 `.dyn`** → 解密并列出条目
...
│   ├── AndroidManifest.xml
│   └── src/com/dsttex/
│       ├── TexConverter.java     # 主界面、纹理转换、预览、动画解析
│       ├── AnimRenderer.java     # 动画渲染(预览与编辑器共用一份实现)
│       └── AnimEditor.java       # 横屏动画编辑器
